    ...
    def make_task(name: 'gig', gemspec_filename: , ignore_files: )
      ...
      Rake::Task.define_task(name) do
        ...
        files.each do |file|
          ...
          if in_spec
            if in_ignore
              file_error.("file is in gig ignore_files, but present in gemspec: #{file}")
            end
            ...
            if in_git
              if in_fs
                ...
                if git_status.empty?
                  ...
                  file_error.("file in gemspec modified from git: #{file}")
                end
              else
            ...
            if in_git
              file_error.("file in git but neither in gemspec nor gig ignore_files: #{file}")
            else
